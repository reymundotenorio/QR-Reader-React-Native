...
import { handleBarCodeScannedProps, stylesQRReaderProps } from '../types';
    ...
    >
      <BarCodeScanner onBarCodeScanned={scanned ? undefined : handleBarCodeScanned} style={[StyleSheet.absoluteFillObject, stylesQRReader({ tabBackgroundColor }).container]}>
        ...
        <TouchableOpacity onPress={toggleFlash} style={stylesQRReader({}).flashButton}>
          {flash && <FlashIcon name='flash' color={'#FFFFFF'} />}
          {!flash && <FlashIcon name='flash-outline' color={'#FFFFFF'} />}
        </TouchableOpacity>
      </BarCodeScanner>
...
const stylesQRReader = ({ tabBackgroundColor }: stylesQRReaderProps) =>
  StyleSheet.create({
    container: {
      flex: 1,
